...
function aerodynamic_force_model_wing_surface(actions, aoa, airspeed, param, lateral_airspeed)
    ...
    """
    crazyflie_model(pwm::Array{Real,1}, param)

pwm are the PWM signals to the motors.

Forces and torques generated by the rotors are calculated.

TODO: Since crazyflie is flying with brushed DC motors, the PWM signal gives a voltage between 0V and battery voltage. a 100% PWM signal will only drive the motor with the battery voltage. With a lower battery voltage, the thrust is also lower. This must be compensated.

"""
function crazyflie_model(pwm)

    # limits for motor pwm signals
    pwm = [ max(0.0, min(pwm[1], 65535.0)),
            max(0.0, min(pwm[2], 65535.0)),
            max(0.0, min(pwm[3], 65535.0)),
    ...
    pitch = (thrust[2] + thrust[3] - thrust[1] - thrust[4] ) * 0.05
    yaw = (torque[1] + torque[3] - torque[2] - torque[4])

    torque_B = [roll; pitch; yaw];
    force_B = [0.0; 0.0; sum(thrust)];
